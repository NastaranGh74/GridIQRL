...
if __name__ == '__main__':
    ...
    all_rewards.to_csv('DuelingDQN_33_500.csv')
    
    #all_loss = pd.DataFrame(all_loss)
    #all_loss.to_csv('DuelingDQN_MSE_loss_33_500.csv')
    
    
    torch.save(agent.q_eval.state_dict(), 'DuelingDQN_33_500.pt')
    
    system_loss = pd.DataFrame(system_loss)
    voltage_penalty = pd.DataFrame(voltage_penalty)
    system_loss.to_csv('system_lossDDQN_33_500.csv')
    voltage_penalty.to_csv('voltage_penaltyDDQN_33_500.csv')
