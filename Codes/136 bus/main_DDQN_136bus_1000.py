...
if __name__ == '__main__':
    ...
    feasible_acts1 = pd.read_csv(data_folder/'sorted_open_lines_136bus.csv', header=0, index_col=0, skiprows=3499000)

    
    env = Reconfig(feasible_acts=feasible_acts1, LINE_DATA=LINE_DATA1, BUS_DATA=BUS_DATA1, P_DATA=P_DATA1 , Q_DATA=Q_DATA1, voltage=13.8)
    ...
    all_rewards.to_csv('DuelingDQN_136_1000.csv')
    
    #all_loss = pd.DataFrame(all_loss)
    #all_loss.to_csv('DuelingDQN_MSE_loss_136_1000.csv')
    
    
    torch.save(agent.q_eval.state_dict(), 'DuelingDQN_136_1000.pt')
    system_loss = pd.DataFrame(system_loss)
    voltage_penalty = pd.DataFrame(voltage_penalty)
    system_loss.to_csv('system_lossDDQN_136_1000.csv')
    voltage_penalty.to_csv('voltage_penaltyDDQN_136_1000.csv')
