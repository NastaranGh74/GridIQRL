...
if __name__ == '__main__':
    ...
    feasible_acts1 = pd.read_csv(data_folder/'sorted_open_lines_136bus.csv', header=0, index_col=0, skiprows=3499600)
    ...
    all_rewards.to_csv('DuelingDQN_136_400.csv')
    
    #all_loss = pd.DataFrame(all_loss)
    #all_loss.to_csv('DuelingDQN_MSE_loss_136_1000.csv')
    
    
    torch.save(agent.q_eval.state_dict(), 'DuelingDQN_136_400.pt')
    system_loss = pd.DataFrame(system_loss)
    voltage_penalty = pd.DataFrame(voltage_penalty)
    system_loss.to_csv('system_lossDDQN_136_400.csv')
    voltage_penalty.to_csv('voltage_penaltyDDQN_136_400.csv')
