...
torch.save(q_network.state_dict(), 'DQN_136_200.pt')

All_Returns = pd.DataFrame(stats['Returns'])
MSE_loss = pd.DataFrame(stats['MSE Loss'])

All_Returns.to_csv('DQN_136_200.csv')
MSE_loss.to_csv('DQN_MSE_loss_136_200.csv')

system_loss = pd.DataFrame(system_loss)
voltage_penalty = pd.DataFrame(voltage_penalty)
system_loss.to_csv('system_lossDQN_136_200.csv')
voltage_penalty.to_csv('voltage_penaltyDQN_136_200.csv')
